...
namespace Dirigent.Remote.iOS.Services.Impl {
	internal sealed class FirebaseDatabaseService : IDatabaseService {
...
Before you continue, please, read this[documentation][10]
to know how Database is structured and best practices for data structure.
...
For basic write operations, you can use SetValue<T> (T value), SetValues<T> (T []
values) or SetValues (NSObject []
values) to save data to a specified reference, replacing any existing data at that path. You can use this method to pass types that correspond to the available JSON types as follows: 
...
| Event type              | Typical usage                                                 | |:-----------------------:|---------------------------------------------------------------| | DataEventType.Value | Read and listen for changes to the entire contents of a path. | 
...
When calling updateChildValues, you can update lower-level child values by specifying a path for the key. If data is stored in multiple locations to scale better, you can update all instances of that data using [data fan-out]
[5]. For example, in notes app, the user wants to save a new note and simultaneously update the total of notes that the folder has.To do this, the code could be like this: 
...
| Method                     | Usage                                                | |:--------------------------:|------------------------------------------------------| | GetQueryOrderedByKey   | Order results by child keys.                         | | GetQueryOrderedByValue | Order results by child values.                       | | GetQueryOrderedByChild | Order results by the value of a specified child key. | 
...
| Method                      | Usage                                                                                                      | |:---------------------------:|------------------------------------------------------------------------------------------------------------| | GetQueryLimitedToFirst  | Sets the maximum number of items to return from the beginning of the ordered list of results.              | | GetQueryLimitedToLast   | Sets the maximum number of items to return from the end of the ordered list of results.                    | | GetQueryStartingAtValue | Return items greater than or equal to the specified key or value, depending on the order-by method chosen. | | GetQueryEndingAtValue   | Return items less than or equal to the specified key or value, depending on the order-by method chosen.    | | GetQueryEqualToValue    | Return items equal to the specified key or value, depending on the order-by method chosen.                 | 
...
1. Children with a key that can be parsed as a 32-bit integer come first, sorted in ascending order. 2. Children with a string value as their key come next, sorted[lexicographically][9] in ascending order. 
...
To learn about Firebase Database best practices, please visit this [Firebase post]
[11]. 
